 ...
 * KED  11-May-2026  Replaced short tags.
 * * * * * * * * * * * * * * * * * * * * * * * */
  ...
  while ($row = mysqli_fetch_array($ssql)) {
    ...
    if (!empty($row[3])) {
      ...
      for ($i = 0; $i < count($charpos); $i++) {
        $pageNo   = ($row[2] + $i + 1);
        $newtext .= trim(substr($row[1], $prevchar, ($charpos[$i] - $prevchar))) .
                    "&nbsp;<span title=\"This paragraph continues on physical Page $pageNo in the printed volume.\" class=\"contNewPage\">$pageNo</span>&nbsp;";
        $prevchar = $charpos[$i];
      }
...
?>
<tr>
  <td valign=top><b><?php echo $row[0]; ?></b></td>
  <td<?php if (!empty($className)) echo " class=\"$className\""; ?>><?php echo $newtext; ?></td>
</tr>
<?php
  }
?>
...
<div align="center" style="padding-top:10px;">
  <form method="POST" id="pageSelector" name="pageSelector">
    ...
    <table border=0 cellpadding=5 cellspacing=0><tr>
      ...
      <td><img width="25" height="25" class="imagebutton" title="Goto Start of this Article (<?php echo $articlePage; ?>)" src="img/article.svg" onclick="ChangePageNo(pageSelector, gotopage, <?php echo $articlePage; ?>);"></td>
      <td width="25">&nbsp;</td>
      ...
      <td width="50">&nbsp;</td>
      <td><img width="25" height="25" class="imagebutton" title="Goto Last Page (<?php echo $volPages; ?>)" src="img/page_last.svg" onclick="ChangePageNo(pageSelector, gotopage, <?php echo $volPages; ?>);"></td>
    </tr></table>
  </form>
</div>
<?php
