...
class ActionUtil {
  ...
  */
  ...
  /**
  * Return the HTML for a GO submit Button
  *
  * @public
  * @returns String
  */
  static function goButton($buttonText='', $tabIndex=-1) {
	  if (empty($buttonText)) {
		  $buttonText = 'GO';
	  }
	  return '<button class="btn-floating btn-large green" type="submit" name="action" '.($tabIndex == -1 ? '' : ' tabindex='.$tabIndex).'  >'.$buttonText.'</button>';
  }
}
?>